...
def createKurs(kurs_id: int, name: str, dozent_id: int):
    """ neuen Kurs in Tabelle 'Kurs' einfügen
        ...
        Args:
            ...
            dozent_id (str): Dozent-ID des Dozenten der dem Kurs der angelegt wird als Studiengangsleiter zugeordnet ist
...
def createVeranstaltung(veranstaltung_id: int, name: str, dozent_id: int, modul_id: int):
    """ neue Veranstaltung in Tabelle 'Veranstaltung' einfügen
        ...
        Args:
            ...
            dozent_id (int): Dozent-ID des Dozenten der die Veranstaltung die angelegt wird unterrichtet
...
def createPruefungsleistung(student_id: int, veranstaltung_id: int, punkte_gesamt: int, punkte_erreicht: int):
    """ neue Prüfungsleistung in Tabelle 'Pruefungsleistung' einfügen
        ...
        Args:
            student_id (int): Studenten-ID des Studenten der die Prüfungleistung die angelegt wird abgelegt hat
            veranstaltung_id (int): Veranstaltung-ID der Veranstaltung in der die Prüfungsleistung die angelegt wird abgelegt wurde
            punkte_gesamt (int): Punkte die in Prüfungsleistung die angelegt wird maximal erreichbar waren
            punkte_erreicht (int): Punkte die in der Prüfungsleisutng die angelegt wird tatsächlich erreicht wurden
...
def deletePruefungsleistung(student_id: int, veranstaltung_id: int):
    """ Prüfungsleistung aus Tabelle 'Pruefungsleistung' entfernen
        ...
        Args:
            student_id (int): Studenten-ID des Studenten der Prüfungsleistung die gelöscht werden soll abgelegt hat
            veranstaltung_id (int): Veranstaltung-ID der Veranstaltung in der Prüfungsleistung die gelöscth werden sol abgelegt wurde
...
def changeKurs(kurs_id_old: int,kurs_id: int, name: str, dozent_id: int):
    """ Attributwerte eines Kurses aus Tabelle 'Kurs' verändern
        ...
        Args:
            ...
            dozent_id (str): neuer Dozent-ID des Dozenten der dem Kurs der verändert wird als Studiengangsleiter zugeordnet ist
...
def changeVeranstaltung(veranstaltung_id_old: int, veranstaltung_id: int, name: str, dozent_id: int, modul_id: int):
    """ Attributwerte einer Veranstaltung aus Tabelle 'Veranstaltung' verändern
        ...
        Args:
            veranstaltung_id_old (int): bisherige Veranstlatung-ID der Veranstaltung die verändert wird
            ...
            dozent_id (int): neue Dozent-ID des Dozenten der die Veranstaltung die verändert wird unterrichtet
            modul_id (int): neue Modul-ID des Moduls zu dem die Veranstaltung die verändert wird gehört
...
def changePruefungsleistung(student_id_old: int, veranstaltung_id_old: int, student_id: int, veranstaltung_id: int, punkte_gesamt: int, punkte_erreicht: int):
    """ Attributwerte einer Prüfungsleistung aus Tabelle 'Pruefungsleistung' verändern
        ...
        Args:
            student_id_old (int): bisherige Studenten-ID des Studenten der die Prüfungsleistung die verändert wird abgelegt hat
            veranstaltung_id_old (int): bisherige Veranstaltung-ID der Veranstaltung in der die Prüfungsleistung die verändert wird abgelegt wurde
            student_id (int): neue Studenten-ID des Studenten der die Prüfungleistung die verändert wird abgelegt hat
            veranstaltung_id (int): neue Veranstaltung-ID der Veranstaltung in der die Prüfungsleistung die verändert wird abgelegt wurde
            punkte_gesamt (int): neue Punkte die in Prüfungsleistung die verändert wird maximal erreichbar waren
            punkte_erreicht (int): neue Punkte die in der Prüfungsleisutng die verändert wird tatsächlich erreicht wurden
...
def getVeranstaltung(veranstaltung_id: int):
    """ Attribute einer Veranstaltung aus Tabelle 'Veranstaltung' abfragen
        ...
        Args:
            veranstaltung_id (int): Veranstaltung-ID der Veranstaltung deren Attributwerte abgefragt werden
...
def getPruefungsleistung(student_id: int, veranstaltung_id: int):
    """ Attribute einer Prüfungsleistung aus Tabelle 'Pruefungsleistung' abfragen
        ...
        Args:
            student_id (int): Studenten-ID des Studenten der Prüfungsleistung abgelegt hat deren Attributwerte abgefragt werden
            veranstaltung_id (int): Veranstlatung-ID der Veranstaltung in der Prüfungsleistung abgelegt wurde deren Attributwerte abgefragt werden
...
def getPruefungsleistungenByStudent(student_id: int):
    """ alle Prüfungsleistungen eines Studenten aus Tabelle 'Pruefungsleistung' abfragen
        ...
        Returns:
            list: Liste mit mit Prüfungsleistungen (entsprechen je: Liste welche Attributwerte der Prüfungsleistungen beinhaltet)
...
if __name__ == '__main__':
    """ Hauptfunktion des Programms, das beim Starten der Schnittstelle bzw. des Servers automatisch ausgeführt wird
        * setzt Datenbank auf
        * befüllt Datenbank mit Testdaten
